...
ecosystem_list AS (
       SELECT
              UNNEST(
                     ARRAY ['Ethereum','Solana','Bitcoin','Hardhat','Solidity','Polygon','Truffle','IPFS','Sui'] :: text []
              ) AS ecosystem_name
),
ecosystem_repos AS (
       SELECT
       ...
       FROM
              ecosystem_list e
              JOIN web3.repos r ON r.upstream_marks ? e.ecosystem_name
),
actor_first_activity AS (
       SELECT
              er.ecosystem_name,
              ev.actor_id,
...
active_dev_ids AS (
       ...
       FROM
              ...
              JOIN web3.event ev ON ev.repo_id = er.repo_id
       WHERE
              ev.event_type IN ('PullRequestEvent', 'PushEvent')
              AND ev.created_at >= NOW() - INTERVAL '3 years'
       GROUP BY
              er.ecosystem_name,
              ev.actor_id
       HAVING
              COUNT(DISTINCT ev.event_type) = 2
),
...
new_developers AS (
       ...
       FROM
              actor_first_activity
       WHERE
              first_activity_time >= NOW() - INTERVAL '90 days'
       GROUP BY
              ecosystem_name
)
SELECT
       el.ecosystem_name AS ecosystem,
       ...
       (
              COALESCE(tp.count, 0) * wc.weight_total + COALESCE(ap.count, 0) * wc.weight_active + COALESCE(nd.count, 0) * wc.weight_new
       ) AS weighted_score
FROM
       ecosystem_list el
