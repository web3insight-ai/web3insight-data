with -- 需要分析的生态
     eco_list as (select distinct unnest(
                                          array [
                                              'Ethereum',
                                              'Bitcoin',
                                              'Solana',
...
-- 活跃开发者：近 1 年内 PR 提交月数大于等于 9 个月的开发者
     active_actor as (select actor_id
                      ...
                      group by actor_id
                      having count(distinct to_char(created_at, 'YYYY-MM')) >= 9),
...
-- 聚合仓库所属生态和所属生态数量
     repo_eco as (select repo_id,
                  ...
                  group by repo_id),
     eco_repo_actor as (select t1.repo_id,
                               t1.actor_id,
                               t2.eco_name
                        from (select repo_id,
                                     actor_id
                              ...
                              group by repo_id,
                                       actor_id) as t1
                                 inner join (select repo_id,
                                                    repo_name,
                                                    unnest(eco_names) as eco_name
                                             from repo_eco) as t2 on t1.repo_id = t2.repo_id
                                 left join bot_list on t1.actor_id = bot_list.actor_id
                        where bot_list.actor_id is null
                        group by t1.repo_id,
                                 t1.actor_id,
                                 t2.eco_name),
     repo_connection as (select t1.eco_name,
                                t1.repo_id,
                                sum(t2.actor_repo_cnt - 1) as connection_cnt
                         from eco_repo_actor as t1
                                  inner join (select eco_name,
                                                     actor_id,
                                                     count(distinct repo_id) as actor_repo_cnt
                                              from eco_repo_actor
                                              group by eco_name,
                                                       actor_id) as t2 on t1.eco_name = t2.eco_name
                             and t1.actor_id = t2.actor_id
                         group by t1.eco_name,
                                  t1.repo_id),
-- 计算仓库得分：
-- - 使用饱和函数对统计量进行处理
-- - 仓库连接数和开发者数量的权重大于行为的权重
-- - 如果一个项目属于多个生态，则对其进行指数衰减：$e^{-0.3 * (eco\_num - 1)}$
     eco_metric as (select eco_name,
                           repo_name,
                           (
                               0.9 * (
                                   0.6 * (1 - exp(-0.01 * connection_cnt :: numeric)) +
                                   0.25 * (1 - exp(-0.1 * active_actor_count :: numeric)) + 0.15 * (
                                       1 - exp(-0.1 * none_active_actor_count :: numeric)
                                       )
                                   ) + 0.1 * (
                                   0.4 * (
                                       1 - exp(-0.001 * active_actor_pr_count :: numeric)
                                       ) + 0.3 * (
                                       1 - exp(-0.001 * none_active_actor_pr_count :: numeric)
                                       ) + 0.15 * (1 - exp(-0.001 * star_count :: numeric)) +
                                   0.15 * (1 - exp(-0.001 * fork_count :: numeric))
                                   )
